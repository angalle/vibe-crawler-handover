...
| # | 분류 | 내용 | 비고 |
|---|------|------|------|
| 1 | 보안 | 인증/인가 체계 미구현 | API 인증 없이 오픈 상태 |
| 2 | 스크립트 관리 | `scripts/` 폴더 구조 재구성 필요 | `shared/routes.py`에 TODO 주석 |
| 3 | 인프라 | AWS CDK 기반 ECS(Fargate) 배포 전환 | 현재 EC2 + docker-compose |
| 4 | 알림 | 카카오톡 알림(Alim Talk) 연동 | 현재 메일만 지원 |
